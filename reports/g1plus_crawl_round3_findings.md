...
> **本节上述内容与「仍未闭环」第 5 项均为本轮当时的历史状态。**该修订已于 2026-09-25 的 **v0.3.11** 原样落地：截至 v0.3.11 的规范哈希为 `765fd8e0…646b60`，只有 FEC 的 `conflict_status` 与 `notes` 两格变化。
>
> **v0.3.12 重新钉定（2026-09-25，历史）**：v0.3.12 规范哈希为 `1b285fe8…22456`；该轮只改 FEC/VC 两行，240 条 `model_ready=true` 行未变。
> **v0.3.13 当前状态（2026-09-25）**：当前规范哈希为 `a446c216…c01085`；v0.3.13 只改 FEC 的 `conflict_status` 与 `notes`（先记 107 腿汇编层读到，再追加下游 40 °C 温度归属冲突），240 条 `model_ready=true` 行仍逐字节未变。
