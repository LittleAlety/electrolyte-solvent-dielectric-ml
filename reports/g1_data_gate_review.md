...
| FEC | 114435-02-8 | **Primary 78.4 landed in v0.3.12; 107 compilation value read in v0.3.13; still held out** | The previously stored 102 was the flash point, not a permittivity. v0.3.12 promoted the 78.4 primary measurement (Kobayashi et al. 2003, Table 2, 296.15 K, `Our data`) to the stored value; the competing 107 compilation value is now read directly in Ue et al. 2014 Table 2.3 (printed p.101), whose body text on p.105 attributes the FEC data to Hagiyama et al. 2008 [25]. Hagiyama 2008 remains unread, so the row stays `model_ready=false` |
